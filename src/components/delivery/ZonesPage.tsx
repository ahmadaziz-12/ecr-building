    ...
    <div className="space-y-4">
      ...
      <SectionCard title="Add zone" desc="Zone name + city + distance + fee. Used in delivery scheduling.">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
          ...
          <Button onClick={() => { if (!f.name) { toast.error("Name is required"); return; } add(f); setF({ name: "", city: "Riyadh", distanceKm: 10, fee: 100 }); toast.success("Zone added"); }} className="bg-brand text-brand-foreground hover:bg-brand/90 md:col-span-4">Add zone</Button>
        </div>
      </SectionCard>
              ...
              {zones.map((z) => (
                <tr key={z.id} className="border-t border-black/5">
                  ...
                  <td className="px-2 py-2 text-right">
                    <button onClick={() => remove(z.id)} className="grid h-7 w-7 place-items-center rounded-md text-muted-foreground hover:bg-critical/10 hover:text-critical">
