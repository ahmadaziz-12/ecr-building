...
export function ProfileMenu({ onSetPin }: { onSetPin: () => void }) {
  ...
  const [dialog, setDialog] = useState<null | "profile" | "password" | "prefs" | "help" | "logout">(null);
  ...
  function savePrefs() {
    ...
    log({ module: "system", event: "USER_PREFERENCES_UPDATED", user: user?.name, severity: "info" });
  ...
  function changePassword() {
    ...
    log({ module: "system", event: "USER_PASSWORD_CHANGED", user: user?.name, severity: "warning" });
      ...
      <Dialog open={dialog === "profile"} onOpenChange={(o) => !o && setDialog(null)}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => setDialog("prefs")} className="bg-brand text-brand-foreground hover:bg-brand/90">
              Edit preferences
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={dialog === "password"} onOpenChange={(o) => !o && setDialog(null)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            <div>
              <Label className="mb-1 block text-xs text-muted-foreground">Current password / PIN</Label>
            ...
            <div>
              <Label className="mb-1 block text-xs text-muted-foreground">Confirm new password / PIN</Label>
              <Input type="password" value={pw.confirm} onChange={(e) => setPw({ ...pw, confirm: e.target.value })} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDialog(null)}>Cancel</Button>
            <Button onClick={changePassword} className="bg-brand text-brand-foreground hover:bg-brand/90">Update password</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={dialog === "prefs"} onOpenChange={(o) => !o && setDialog(null)}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Pref label="Language" value={prefs.language} options={["English", "العربية"]} onChange={(v) => setPrefs({ ...prefs, language: v })} />
            <Pref label="Theme" value={prefs.theme} options={["Light", "Dark", "System"]} onChange={(v) => setPrefs({ ...prefs, theme: v })} />
          ...
          <div className="space-y-2 rounded-lg border border-black/5 p-3 text-sm">
            ...
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={prefs.notifyApprovals} onChange={(e) => setPrefs({ ...prefs, notifyApprovals: e.target.checked })} />
              Approval requests awaiting me
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={prefs.notifyLowStock} onChange={(e) => setPrefs({ ...prefs, notifyLowStock: e.target.checked })} />
              Low-stock and reorder alerts
            </label>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDialog(null)}>Cancel</Button>
            <Button onClick={savePrefs} className="bg-brand text-brand-foreground hover:bg-brand/90">Save preferences</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={dialog === "help"} onOpenChange={(o) => !o && setDialog(null)}>
        <DialogContent className="max-w-md">
          ...
          <ul className="space-y-2 text-sm">
            <li>• <span className="font-medium">Keyboard:</span> F2 search products, F4 payment, Esc closes dialogs.</li>
            <li>• <span className="font-medium">Stock questions:</span> Products &amp; Stock → Stock Locations / Inventory &amp; Stock.</li>
            <li>• <span className="font-medium">Approvals:</span> Operate → Approval Center.</li>
            <li>• <span className="font-medium">Support:</span> support@mimoney.example.sa · +966 55 000 0000</li>
          </ul>
          <DialogFooter>
...
function Pref({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (v: string) => void }) {
  return (
    <div>
