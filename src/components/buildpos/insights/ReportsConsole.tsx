...
function ReportCatalog({ onOpen }: { onOpen: (key: ReportKey) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {shownGroups.map((g) => (
        <div key={g}>
          <p className="mb-2 px-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">{g}</p>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {matches.filter((r) => r.group === g).map((r) => {
              const Icon = r.icon;
              ...
              return (
                ...
                >
                  <span className={`grid h-10 w-10 flex-none place-items-center rounded-xl ${tone.tile}`}>
      ...
      {matches.length === 0 && (
        <SectionCard>
          <p className="py-10 text-center text-sm text-muted-foreground">No report matches “{search}”.</p>
        </SectionCard>
      )}
    </div>
  );
    ...
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-brand">Insights · {def.group}</p>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="ghost" size="sm" onClick={onBack} className="h-9 gap-1.5 text-muted-foreground hover:text-foreground">
...
const KPI_BUILDERS: Partial<Record<ReportKey, (rows: never[]) => Kpi[]>> = {
  "top-products": (rows) => [
    ...
    { label: "Revenue", value: money(sum(rows, "revenue")), sub: "Ex-VAT, after discount", tone: "success" },
    { label: "Gross Profit", value: money(sum(rows, "grossProfit")), sub: "Revenue − COGS", tone: "success" },
    { label: "Discounts", value: money(sum(rows, "discounts")), sub: "Off list price", tone: "warning" },
    { label: "Returned Units", value: int(sum(rows, "returnedUnits")), sub: "Came back in period", tone: "warning" },
    { label: "Share Covered", value: `${sum(rows, "sharePct").toFixed(1)}%`, sub: "Of filtered revenue", tone: "muted" },
    { label: "Out of Stock", value: int(rows.filter((r) => (r as { onHand: number }).onHand <= 0).length), sub: "Selling with no cover", tone: rows.some((r) => (r as { onHand: number }).onHand <= 0) ? "critical" : "muted" },
  ],
  "item-report": (rows) => [
  ...
  "employee-report": (rows) => [
    ...
    { label: "Net Sales Rung", value: money(sum(rows, "netSales")), sub: "Across all accounts", tone: "success" },
    { label: "Discounts Given", value: money(sum(rows, "discounts")), sub: "At the register", tone: "warning" },
    { label: "Voids", value: int(sum(rows, "voidedOrders")), sub: "Orders reversed", tone: sum(rows, "voidedOrders") > 0 ? "critical" : "muted" },
    { label: "Refunds Approved", value: int(sum(rows, "refundsApproved")), sub: money(sum(rows, "refundValue")), tone: "warning" },
    { label: "Cash Variance", value: money(sum(rows, "cashVariance")), sub: "Across closed shifts", tone: sum(rows, "cashVariance") !== 0 ? "critical" : "success" },
    { label: "Audit Events", value: int(sum(rows, "auditEvents")), sub: "Logged actions", tone: "info" },
    { label: "Critical Events", value: int(sum(rows, "criticalEvents")), sub: "Needs review", tone: sum(rows, "criticalEvents") > 0 ? "critical" : "muted" },
  ],
  "employee-audit": (rows) => [
