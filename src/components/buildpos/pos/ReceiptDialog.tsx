...
export function ReceiptDialog({ order, terminalId, onClose }: { order: OrderDto | null; terminalId: number | null; onClose: () => void }) {
  ...
  const qzPrinterName = devices?.find((d) => d.terminalId === terminalId && d.type === "ReceiptPrinter")?.qzPrinterName ?? null;
  ...
  async function handlePrint() {
    ...
    try {
      await dispatchPrint();
      toast.success(
        qzIsConnected() && qzPrinterName ? "Sent to physical receipt printer" : "Sent to receipt printer",
        { description: qzIsConnected() && qzPrinterName ? `Via QZ Tray → ${qzPrinterName}` : "Rendered as ESC/POS bytes (virtual printer)." },
      );
    } catch (err) {
      ...
      <DialogContent className="max-w-sm p-0">
        ...
        <div className="max-h-[60vh] overflow-y-auto px-5 py-4 font-mono text-xs text-foreground">
          ...
          <div className="mt-3 space-y-0.5 border-b border-dashed border-black/20 pb-2">
            <div className="flex justify-between"><span>Order</span><span>{order.orderNo}</span></div>
            <div className="flex justify-between"><span>Date</span><span>{new Date(order.createdAt).toLocaleString("en-GB", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</span></div>
          ...
          <div className="mt-2 space-y-0.5 border-b border-dashed border-black/20 pb-2">
            ...
            <div className="flex justify-between"><span>VAT</span><span>{money(order.vatTotal)}</span></div>
            <div className="flex justify-between text-sm font-bold"><span>TOTAL</span><span>{money(order.grandTotal)}</span></div>
          </div>
          ...
          <div className="mt-3 flex flex-col items-center gap-1">
            {invoice?.qrCodeBase64 ? (
              <>
                ...
                <p className="text-[10px] text-muted-foreground">ZATCA Cleared · {invoice.invoiceNumber}</p>
              </>
            ) : (
        ...
        <div className="flex items-center justify-between gap-2 border-t border-black/5 p-3">
          ...
          <Button size="sm" onClick={handlePrint} disabled={printReceipt.isPending} className="gap-1.5 bg-brand text-brand-foreground hover:bg-brand/90">
            {printReceipt.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Printer className="h-4 w-4" />} Print
          </Button>
        </div>
      </DialogContent>
    </Dialog>
