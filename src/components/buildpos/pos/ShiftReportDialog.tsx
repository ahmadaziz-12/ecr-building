        ...
        {report && (
          <div className="space-y-3 text-sm" id="shift-report-printable">
            ...
            <div className="grid grid-cols-2 gap-y-1">
              <span className="text-muted-foreground">Opening Float</span><span className="text-right">{fmtSar(report.openingFloat)}</span>
              <span className="text-muted-foreground">Cash Sales</span><span className="text-right">{fmtSar(report.cashSales)}</span>
              <span className="text-muted-foreground">Cash In/Out</span><span className="text-right">{fmtSar(report.cashIn - report.cashOut)}</span>
              <span className="font-semibold">Expected Cash</span><span className="text-right font-semibold">{fmtSar(report.expectedCash)}</span>
              {report.countedCash !== null && (
                <>
                  <span className="text-muted-foreground">Counted Cash</span><span className="text-right">{fmtSar(report.countedCash)}</span>
                  <span className="text-muted-foreground">Variance</span><span className="text-right">{report.variance?.toFixed(2)} ر.س</span>
                </>
              )}
            </div>
            <div>
              ...
              <div className="rounded-lg border border-black/5">
                {report.paymentBreakdown.map((b) => (
                  <div key={b.method} className="flex justify-between border-b border-black/5 px-3 py-1.5 last:border-0">
                    ...
                    <span className="font-medium">{fmtSar(b.amount)}</span>
                  </div>
                ))}
