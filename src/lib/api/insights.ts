...
export type AdminOverviewDto = {
  ...
  areas: OverviewAreaDto[];
};
export type SavedDashboardViewDto = {
  id: number;
  name: string;
  filtersJson: string;
  createdAt: string;
...
export const useInsightsSales = (enabled = true) =>
  useQuery({
    queryKey: ["insights", "sales"],
    queryFn: () => apiGet<SalesSegmentDto[]>("/api/insights/sales"),
    enabled,
  });
export const useInsightsKpi = (enabled = true) =>
  useQuery({
    queryKey: ["insights", "kpi"],
    queryFn: () => apiGet<KpiDto[]>("/api/insights/kpi"),
    enabled,
  });
export const useInsightsReports = (enabled = true) =>
  useQuery({
    queryKey: ["insights", "reports"],
    queryFn: () => apiGet<ReportDefinitionDto[]>("/api/insights/reports"),
    enabled,
  });
export const useInsightsBi = (enabled = true) =>
  useQuery({
    queryKey: ["insights", "bi"],
    queryFn: () => apiGet<BiFeedDto[]>("/api/insights/bi"),
    enabled,
  });
export const useAdminOverview = (enabled = true) =>
  useQuery({
    queryKey: ["admin", "overview"],
    queryFn: () => apiGet<AdminOverviewDto>("/api/admin/overview"),
    enabled,
  });
...
export function useUpdateKpiTarget() {
  ...
  return useMutation({
    mutationFn: ({ id, target }: { id: number; target: number }) =>
      apiPut(`/api/insights/kpi/${id}/target`, { target }),
...
export function useSetReportStatus() {
  ...
  return useMutation({
    mutationFn: ({ id, status }: { id: number; status: "Active" | "Inactive" }) =>
      apiPut<ReportDefinitionDto>(`/api/insights/reports/${id}/status`, { status }),
...
export const useSavedViews = (enabled = true) =>
  useQuery({
    queryKey: ["insights", "saved-views"],
    queryFn: () => apiGet<SavedDashboardViewDto[]>("/api/insights/saved-views"),
    enabled,
  });
...
export function useDeleteSavedView() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: number) => apiDelete(`/api/insights/saved-views/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["insights", "saved-views"] }),
  });
}
