...
export type VatByRateRow = {
  rate: number; rateLabel: string; orders: number; taxableAmount: number; vatCollected: number;
  vatReversed: number; netVat: number; sharePct: number;
};
export type VatReportDto = {
  taxableSales: number; totalCollected: number; totalReversed: number; netVat: number;
...
export type ItemReportRow = {
  productId: number; sku: string; barcode: string | null; name: string; category: string; brand: string | null;
  supplier: string | null; uom: string; costPrice: number; sellingPrice: number; vatRate: number; marginPct: number;
  warehouseOnHand: number; branchOnHand: number; reserved: number; available: number; reorderLevel: number;
  stockValue: number; retailValue: number; unitsSold: number; revenue: number; cogs: number; grossProfit: number;
  returnedUnits: number; returnRatePct: number; lastSoldAt: string | null; stockStatus: string; status: string;
};
export type InventoryReportRow = {
  productId: number; sku: string; name: string; category: string; locationType: string; locationId: number;
  location: string; branch: string; onHand: number; reserved: number; available: number; reorderLevel: number;
  costPrice: number; stockValue: number; retailValue: number; lastMovementAt: string | null; status: string;
};
export type LowStockRow = {
  productId: number; sku: string; name: string; category: string; locationType: string; locationId: number;
  location: string; branch: string; onHand: number; reserved: number; available: number; reorderLevel: number;
  reorderQty: number; shortfall: number; suggestedOrderQty: number; supplier: string | null; leadTimeDays: number;
  costPrice: number; restockValue: number; status: string;
};
export type StockCountVarianceLine = {
  sku: string; product: string; category: string; systemQty: number; countedQty: number | null; variance: number;
  unitCost: number; varianceValue: number; lineStatus: string; note: string | null;
};
export type StockCountVarianceRow = {
  id: number; countNo: string; date: string; warehouse: string; branch: string; scope: string; category: string | null;
  status: string; countedBy: string | null; approvedBy: string | null; lines: number; counted: number;
  varianceLines: number; netVarianceQty: number; netVarianceValue: number; absVarianceValue: number;
  accuracyPct: number; items: StockCountVarianceLine[];
};
export type ExpiryReportRow = {
  batchId: number; sku: string; product: string; category: string; warehouse: string; branch: string; batchNo: string;
  receivedDate: string; expiryDate: string; daysLeft: number; qty: number; unitCost: number; value: number; status: string;
};
export type PurchaseOrderReportLine = {
  sku: string; product: string; category: string; branch: string; warehouse: string | null; uom: string;
  qty: number; receivedQty: number; pendingQty: number; unitCost: number; lineTotal: number;
  batchNo: string | null; expiryDate: string | null;
};
export type PurchaseOrderReportRow = {
  id: number; poNo: string; orderedAt: string; expectedDate: string; supplier: string; status: string;
  currency: string; branches: string; carrier: string | null; trackingRef: string | null; approvedBy: string | null;
  itemCount: number; qtyOrdered: number; qtyReceived: number; qtyPending: number; receivedPct: number;
  subTotal: number; shipping: number; total: number; daysLate: number; items: PurchaseOrderReportLine[];
};
export type SupplierReturnReportLine = {
  sku: string; product: string; category: string; qty: number; unitCost: number; lineValue: number; batchNo: string | null;
};
export type SupplierReturnReportRow = {
  id: number; rtsNo: string; date: string; supplier: string; branch: string; warehouse: string; linkedPo: string | null;
  reason: string; status: string; creditNoteRef: string | null; carrier: string | null;
  itemCount: number; qty: number; value: number; items: SupplierReturnReportLine[];
};
export type SupplierPerformanceRow = {
  supplierId: number; supplier: string; type: string; terms: string; leadTimeDays: number; poCount: number;
  orderedValue: number; receivedValue: number; fillRatePct: number; onTimePct: number; avgLeadTimeDays: number;
  returnCount: number; returnedValue: number; returnRatePct: number; outstanding: number;
};
export type CustomerReturnReportLine = {
  sku: string; product: string; category: string; qty: number; unitPricePaid: number; vatRate: number; amount: number;
};
export type CustomerReturnReportRow = {
  id: number; returnNo: string; date: string; type: string; orderNo: string | null; customer: string | null;
  branch: string; approvedBy: string | null; reason: string; damageReason: string | null; refundMethod: string;
  status: string; dgrnNo: string | null; itemCount: number; qty: number; grossRefund: number; vatReversal: number;
  restockingFee: number; netCashback: number; items: CustomerReturnReportLine[];
};
export type DamagedItemReportRow = {
  returnId: number; returnNo: string; dgrnNo: string | null; date: string; sku: string; product: string;
  category: string; branch: string; customer: string | null; qty: number; unitCost: number; lossValue: number;
  damageReason: string; notes: string | null; photoReference: string | null; approvedBy: string | null; status: string;
};
export type EmployeeAuditRow = {
  id: number; date: string; userName: string | null; employeeName: string | null; department: string | null;
...
export type ProfitMarginRow = {
  key: string; label: string; sub: string | null; unitsSold: number; revenue: number; cogs: number;
  grossProfit: number; marginPct: number; discounts: number; returns: number; netProfit: number;
};
export type PaymentMethodReportRow = {
  method: string; transactions: number; amount: number; sharePct: number; refunds: number; refundAmount: number; net: number;
};
export type CategoryPerformanceRow = {
  categoryId: number; category: string; skuCount: number; unitsSold: number; revenue: number; cogs: number;
  grossProfit: number; marginPct: number; returnedUnits: number; returnRatePct: number; onHand: number; stockValue: number;
};
export type SurplusReturnReportRow = {
  returnId: number; returnNo: string; date: string; orderNo: string | null; soldAt: string | null;
  daysHeld: number | null; sku: string; product: string; category: string; uom: string; branch: string;
  customer: string | null; qty: number; unitPricePaid: number; refundAmount: number; unitCost: number;
  restockValue: number; restockingFeePct: number; restockingFee: number; netCashback: number;
  refundMethod: string; reason: string; approvedBy: string | null; status: string;
};
export type DeliveryReportLine = {
  sku: string; product: string; category: string; uom: string; ordered: number; loaded: number;
  delivered: number; missing: number; damaged: number; unitWeight: number;
};
export type DeliveryReportRow = {
  id: number; deliveryNo: string; date: string; orderNo: string | null; branch: string; customer: string | null;
  project: string | null; poRef: string | null; area: string; city: string; priority: string; stage: string;
  paymentStatus: string; driver: string | null; vehicle: string | null;
  promisedDate: string; promisedTime: string; timeSlot: string | null;
  dispatchedAt: string | null; deliveredAt: string | null; cycleHours: number | null; daysLate: number;
  punctuality: string; weightTons: number; itemCount: number; qtyOrdered: number; qtyLoaded: number;
  qtyDelivered: number; qtyMissing: number; qtyDamaged: number; fulfilledPct: number;
  amount: number; deliveryFee: number; handlingCharge: number; heavyCharge: number; discountCharge: number;
  vatCharge: number; totalCharges: number; receivedBy: string | null; proof: string | null;
  failureReason: string | null; notes: string | null; items: DeliveryReportLine[];
};
export type DriverPerformanceRow = {
  driverId: number; driver: string; branch: string; vehicle: string | null; status: string; licenseExpiry: string;
  deliveries: number; delivered: number; failed: number; cancelled: number; inFlight: number;
  onTimePct: number; avgCycleHours: number; avgDaysLate: number; tonnageDelivered: number;
  deliveredValue: number; feeRevenue: number; qtyDelivered: number; qtyMissing: number; qtyDamaged: number;
  fulfilmentPct: number; lastDeliveryAt: string | null;
};
export type ShiftCashMovementRow = { at: string; direction: string; amount: number; reason: string; by: string | null };
export type ShiftReportRow = {
  id: number; openedAt: string; closedAt: string | null; durationHours: number; terminal: string; branch: string;
  cashier: string; status: string; openingFloat: number; cashSales: number; cashIn: number; cashOut: number;
  expectedCash: number; countedCash: number | null; variance: number; cashResult: string;
  orders: number; grossSales: number; discounts: number; vat: number; netTakings: number;
  nonCashTakings: number; itemsSold: number; avgBasket: number; voidedOrders: number; voidedValue: number;
  refunds: number; refundValue: number; cashMovements: number; items: ShiftCashMovementRow[];
};
...
export type ReportQuery = ReportDateRange & Record<string, string | number | boolean | string[] | undefined>;
