...
// the endpoint level (every authenticated role should see SOMETHING relevant to them); each item
// is only included if the caller can View the specific page that notification links to, so a
// Cashier never sees HR/Admin-only alerts they have no access to act on.
[ApiController]
[Route("api/notifications")]
[Authorize]
public class NotificationsController(AppDbContext db, IPermissionResolver permissionResolver) : ControllerBase
{
    [HttpGet]
    public async Task<ActionResult<List<NotificationDto>>> List(CancellationToken ct)
    {
        var userId = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        async Task<bool> Can(string page) => await permissionResolver.HasAsync(userId, page, PermissionAction.View, ct);

        var now = DateTime.UtcNow;
        var notifications = new List<NotificationDto>();

        if (await Can("/stock/stocks"))
        {
        ...
        if (await Can("/stock/branch-stock"))
        {
        ...
        if (await Can("/admin/maintenance"))
        {
        ...
        if (await Can("/admin/zatca-invoices"))
        {
