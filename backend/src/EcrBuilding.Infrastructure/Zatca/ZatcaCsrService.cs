...
// Ported from the baqala-bright-flow reference (itself a port of ZATCA's PHP CsrGenerator.php,
// verified end-to-end against ZATCA's live developer-portal sandbox): an EC key on secp256k1 +
// a CSR whose SAN carries a ZATCA-specific directoryName (SN/UID/title/registeredAddress/
// businessCategory) and a private certificateTemplateName extension. .NET's
// CertificateRequest/SubjectAlternativeNameBuilder can't express a directoryName SAN, hence
// BouncyCastle.
public class ZatcaCsrService : IZatcaCsrService
{
    ...
    public ZatcaCsrResult GenerateCsr(ZatcaCsrConfig config)
    {
        ...
        // inspection): CN = EGS Serial, certificateTemplateName encoded as PrintableString with
        // the environment prefix — actually gets a 200 with a real issued certificate reflecting
        // the submitted CSR. Don't "fix" this to match ZATCA's Swagger documentation example
        // (unprefixed UTF8String templateName) without a live round-trip test — that example is a
        // static canned response, not real validation.
    ...
    {
        // SEC1 "EC PRIVATE KEY" structure, rather than PKCS8 — includes the named curve OID and
