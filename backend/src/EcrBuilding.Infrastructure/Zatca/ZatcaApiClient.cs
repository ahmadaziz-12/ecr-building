...
// Real calls to ZATCA's e-invoicing gateway (ported from the baqala-bright-flow reference, itself
// a port of ApiHelper.php), including its retry/backoff shape (3 attempts, exponential backoff,
// treats HTTP 200/201/202 as success).
public class ZatcaApiClient(HttpClient httpClient, ILogger<ZatcaApiClient> logger) : IZatcaApiClient
{
    ...
    private static string EnvironmentSegment(string environment) => environment switch
    {
        ...
        _ => "developer-portal", // NonProduction
    };
        ...
        bool clearanceStatusHeader = false, int retries = 3, int backoffSeconds = 1)
    {
        for (var attempt = 0; attempt < retries; attempt++)
        {
            ...
            // ZATCA's gateway is strict about this — a bare "Content-Type: application/json" with
            // no charset param; StringContent adds one by default, which some strict gateways
            // treat as a malformed/unrecognized request.
