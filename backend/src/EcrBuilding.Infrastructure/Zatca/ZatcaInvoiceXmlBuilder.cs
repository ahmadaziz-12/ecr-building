...
// Ported from the baqala-bright-flow reference (itself a port of InvoiceHelper.php's ModifyXml /
// UpdateInvoiceFromData): fills the base UBL 2.1 Invoice.xml template with per-transaction data.
// Namespaces/structure must stay byte-compatible with the template since
// ZatcaInvoiceSigner queries it by exact XPath afterwards.
public class ZatcaInvoiceXmlBuilder
{
    ...
    private static XmlDocument LoadTemplate()
    {
        ...
        using var stream = assembly.GetManifestResourceStream("EcrBuilding.Infrastructure.Zatca.Templates.Invoice.xml")
