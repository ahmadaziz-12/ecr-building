...
// Ported from the baqala-bright-flow reference (itself a port of EInvoiceSigner.php +
// QRCodeGenerator.php).
//
...
public class ZatcaInvoiceSigner
{
    ...
    private static string ApplyXsltAndCanonicalize(XmlDocument doc)
    {
        ...
        using (var xsltStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("EcrBuilding.Infrastructure.Zatca.Templates.xslfile.xsl")!)
    ...
    // Matches openssl_sign($hashBytes, ..., OPENSSL_ALGO_SHA256): the signer itself applies
    ...
    private static string GenerateQrCode(string canonicalXml, string invoiceHash, string signatureValue, byte[] certDer, string certificateContentBase64)
    {
        ...
        // Tags 6/7 carry the base64 TEXT of the hash/signature (matches the reference exactly —
    ...
    {
        using var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream($"EcrBuilding.Infrastructure.Zatca.Templates.{fileName}")
